...
document
  .querySelector('.member-button')
  .addEventListener('click', memberClickHandler);
...
const getFbProfile = () => {
  FB.api(
    '/me',
    'GET',
    { fields: 'name, email, picture.width(720).height(720)' },
    (user) => renderMemberProfile(user),
  );
};
  ...
  FB.getLoginStatus((res) => {
    if (res.status === 'connected') {
      ...
      if (
        ...
        || document.querySelector('.delivery-payment')
      ) {
        getFbProfile();
      }
